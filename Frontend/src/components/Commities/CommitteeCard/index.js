...
const CommitteeCard = ({ data }) => {
  ...
  const committeee_name = data.name
  const categories = data.categories

  const FormData = { committeee_name, categories }
  ...
  return (
    <div>
      ...
      <div className=" p-6 sm:relative md:left-0 sm:left-0 z-0">
        {showForm && <FormCard Data={FormData} />}
      </div>
    </div>
  )
}
